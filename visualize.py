...
class Visualize(object):
	...
	def combine_images(self, pred, origin, name, save_dir):
...
def get_args():
    ...
    parser.add_argument('--dataset', default='pascal', type=str)
    parser.add_argument('--original-dir', type=str)
    parser.add_argument('--prediction-dir', type=str)
    parser.add_argument('--save-dir', default='./prd', type=str)
    return parser.parse_args()

if __name__ == '__main__':
    args = get_args()
    vis = Visualize(args.dataset)
    preds = []
    for pred in os.listdir(args.prediction_dir):
        preds.append( os.path.join (args.prediction_dir, pred) )
    for pred in preds:
        name = os.path.basename( pred )
        origin_name = name.split('.')[0] + ".jpg"
        _pred = Image.open(pred).convert("RGB")
        _origin = Image.open( os.path.join(args.original_dir, origin_name) ).convert("RGB")
        vis.combine_images(_pred, _origin, name, args.save_dir)
